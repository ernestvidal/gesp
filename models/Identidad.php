...
class Identidad extends \yii\db\ActiveRecord {
    ...
    public function validar_dni($attribute) {
        $dni = $this->$attribute;
        $letra = substr($dni, -1);
    ...
    public function isValidNIF($attribute) {
        $docNumber = $this->$attribute;
        $isValid = FALSE;
        $fixedDocNumber = "";
        $correctDigit = "";
    ...
    public function isValidCIFFormat($docNumber) {
        return
                $this->respectsDocPattern($docNumber, '/^[PQSNWR][0-9][0-9][0-9][0-9][0-9][0-9][0-9][A-Z0-9]/')
                or
    ...
    public function getCIFCheckDigit($docNumber) {
        ...
        if ($this->isValidCIFFormat($fixedDocNumber)) {
            ...
            $oddSum = $this->sumDigits(substr($centralChars, 0, 1) * 2) +
    ...
    public $identificationType = array(
