...
function borrar(item){
  ...
  if (confirm('Borrar línea ?')) {
	var id = ('#line_'+item);
        $(id).remove();
  	$('input').trigger('keyup'); //disparamos el evento para que recalcule la albaran.
  }else{
  return false;
  }
}
 ...
 $(document).ready(function(){
     ...
     $(function(){
    //get the click of sendAlbaranButton on albaran/index and show modal
    $('.sendAlbaranButton').click(function(){
        $('#modalSendAlbaran').modal('show')
                .find('#modalContent')
    ...
    /*
    * hacemos una delegación de evento para que las líneas de albaran que creemos despues del evento domready,
    ...
    */
    ...
    $('form').delegate('input', 'keyup', function(){
        //$(this).keyup(function(){
            ...
            var subtotal = 0;
            var fila = $('#item_line .albaran_line');
                fila.each(function(i, el) {
                    ...
                    var albaran_num = $('#albaran_num').val();
                    $("#item_total_" + i).val(total_linea.toFixed(2));
                    //$('#item_cantidad_' + i).val(cantidad);
                    //$('#item_precio_' + i).val(precio);
                    $('#albaran_item_num_' + i).val(albaran_num);

                    base_imponible += parseFloat($("#item_total_" + i).val());
                    $('#albaran_base_imponible').val(base_imponible.toFixed(2));

                    $('#albaran_importe_descuento').val(((parseFloat($('#albaran_base_imponible').val()) * (parseFloat($('#albaran_rate_descuento').val()) / 100)) * -1).toFixed(2) );
                    $('#albaran_importe_iva').val((parseFloat($('#albaran_base_imponible').val()) * (parseFloat($('#albaran_rate_iva').val()) / 100)).toFixed(2));
                    $('#albaran_importe_irpf').val(((parseFloat($('#albaran_base_imponible').val()) * (parseFloat($('#albaran_rate_irpf').val()) / 100)) * -1).toFixed(2));
                    $('#albaran_total').val((parseFloat($('#albaran_base_imponible').val()) + parseFloat($('#albaran_importe_descuento').val()) + parseFloat($('#albaran_importe_iva').val()) +
                            parseFloat($('#albaran_importe_irpf').val())).toFixed(2));
    ...
    $('#add_item_line').click(function(){
        ...
        var new_input_cantidad = $('<input>', {
            ...
            'name'  : 'AlbaranItem[' + item_order + '][item_cantidad]',
        ...
        var new_textarea = $('<textarea>', {
            ...
            'name' : 'AlbaranItem[' + item_order + '][item_descripcion]',
            'class' : 'form-control',
        });
        ...
        var new_input_precio = $('<input>', {
            ...
            'name'  : 'AlbaranItem[' + item_order + '][item_precio]',
            'class' : 'form-control text-right',
            'value' : '0.00'
        });
        ...
        var new_div_albaran_item_num =  $('<div/>', {
            'class' : 'col-md-1',
        });

         var new_input_albaran_item_num = $('<input>', {
            'type'  : 'hidden',
            'id'    : 'albaran_item_num_' + item_order,
            'name'  : 'AlbaranItem[' + item_order + '][albaran_num]',
        });
        ...
        $(new_input_total).append( new_div_albaran_item_num);
        $(new_div_albaran_item_num).append(new_input_albaran_item_num);
